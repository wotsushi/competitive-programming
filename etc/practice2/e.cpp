...
int main() {
  ...
  vvll(A, N);
  ll M = 0;
  rep(i, N) { ch(max, M, *max_element(ALL(A[i]))); }
  ll src = N * N + 2 * N;
  ll sink = N * N + 2 * N + 1;
  mcf_graph<ll, ll> G(N * N + 2 * N + 2);
  rep(i, N) {
    rep(j, N) {
      G.add_edge(N * N + i, N * i + j, 1, M - A[i][j]);
      G.add_edge(N * i + j, N * N + N + j, 1, 0);
    }
  }
  rep(i, N) { G.add_edge(src, N * N + i, K, 0); }
  rep(j, N) { G.add_edge(N * N + N + j, sink, K, 0); }
  mcf_graph<ll, ll> tmp = G;
  ll opt = 0;
  ll f_opt;
  for (auto [f, c] : G.slope(src, sink)) {
    ll s = f * M - c;
    if (s > opt) {
      opt = s;
      f_opt = f;
