...
int main() {
  ...
  vin(string, S, N);
  mf_graph<ll> G(2 + N * M);
  auto f = [&](ll i, ll j) { return M * i + j + 1; };
  rep(i, N) {
    rep(j, M) {
      if ((i + j) & 1) {
        way4(i, j, N, M, [&](ll u, ll v) { G.add_edge(f(u, v), f(i, j), 1); });
        G.add_edge(f(i, j), N * M + 1, S[i][j] == '.');
      } else {
        G.add_edge(0, f(i, j), S[i][j] == '.');
      }
    }
  }
  out(G.flow(0, N * M + 1));
  auto E = G.edges();
  auto g = [&](ll c) { return ii((c - 1) / M, (c - 1) % M); };
  each(e, E) {
    if (e.flow and 1 <= e.from and e.from <= N * M and 1 <= e.to and
        e.to <= N * M) {
      auto [i, j] = g(e.from);
      auto [u, v] = g(e.to);
      if (i == u) {
