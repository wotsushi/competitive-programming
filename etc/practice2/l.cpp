...
S op(S a, S b) {
  auto [a0, a1, a2] = a;
  auto [b0, b1, b2] = b;
  return {a0 + b0, a1 + b1, a2 + b2 + a1 * b0};
}
S e() { return {0, 0, 0}; }
using F = bool;
S mapping(F f, S x) {
...
F composition(F f, F g) { return f xor g; }
F id() { return false; }
...
int main() {
  ...
  vll(T, L, R, Q);
  vec<S> B(N);
  rep(i, N) { B[i] = {A[i] == 0, A[i] == 1, 0}; }
  lazy_segtree<S, op, e, F, mapping, composition, id> segt(B);
  rep(i, Q) {
    if (T[i] == 1) {
      segt.apply(L[i] - 1, R[i], true);
    } else {
      out(get<2>(segt.prod(L[i] - 1, R[i])));
    }
  }
}
