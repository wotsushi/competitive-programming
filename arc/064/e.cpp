...
template <typename W>
vector<W> dijkstra(graph<W> &G, ll s)
{
    using E = pair<W, ll>;
    vector<W> dp(G.n + 1, INF);
    pq_asc<E> q;
    q.push({0, s});
    dp[s] = 0;
    while (!q.empty())
...
int main()
{
    ...
    auto d = [&](ll x_1, ll y_1, ll x_2, ll y_2) {
        return sqrt(pow(x_2 - x_1, 2) + pow(y_2 - y_1, 2));
    };

    graph<double> G(N + 2, 0, true);
    REP(i, N)
    {
        for (ll j = i + 1; j < N; ++j)
        {
            G.add(i, j, max(0.0, d(x[i], y[i], x[j], y[j]) - r[i] - r[j]));
        }
        G.add(i, N, max(0.0, d(x[i], y[i], x_s, y_s) - r[i]));
        G.add(i, N + 1, max(0.0, d(x[i], y[i], x_t, y_t) - r[i]));
    }
    G.add(N, N + 1, d(x_s, y_s, x_t, y_t));
    auto dp = dijkstra(G, N);
    FOUT(dp[N + 1]);
}