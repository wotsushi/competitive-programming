...
int main() {
  ...
  vll(p, N);
  vec<segt<ii>> rmq(
      2, segt<ii>(N, {INF, 0}, [&](ii x, ii y) { return min(x, y); }));
  rep(i, N) { rmq[i & 1].update(i, {p[i], i}); }
  pq<tuple<ii, ll, ll>> q;
  q.push({rmq[0].query(0, N), 0, N});
  vi ans(N);
  ll k = 0;
  while (not q.empty()) {
    auto [t, i, j] = q.top();
    ...
    ans[k] = x;
    ++k;
    auto [y, r] = rmq[(i + 1) & 1].query(l + 1, j);
    ans[k] = y;
    ++k;
    if (i < l) {
      q.push({rmq[i & 1].query(i, l), i, l});
    }
    if (l + 1 < r) {
      q.push({rmq[(l + 1) & 1].query(l + 1, r), l + 1, r});
    }
    if (r + 1 < j) {
      q.push({rmq[(r + 1) & 1].query(r + 1, j), r + 1, j});
    }
  }
  jout(ALL(ans)) << endl;
}
